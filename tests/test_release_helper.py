...
def _release_checkout(tmp_path: Path, *, init_version: str = "2.5.10") -> Path:
...
def test_release_helper_accepts_metadata_free_setup_shim(tmp_path: Path) -> None:
    ...
    assert "--push requires the existing signed local tag v2.5.10" in result.stderr
...
def test_release_helper_rejects_init_version_mismatch(tmp_path: Path) -> None:
    ...
    assert "pyproject=2.5.10" in result.stderr
