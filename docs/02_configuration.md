...
| `SUPERTABLE_DUCKDB_WRITE_PROBE` | bool | `false` | Legacy compatibility switch: explicitly enable the DuckDB overwrite/delete probe for remote storage; LocalStorage uses the Island-native probe. `false` leaves remote storage on Polars and does not disable the local-auto lane |
| `SUPERTABLE_DUCKDB_WRITE_PROBE_LOCAL_AUTO` | bool | `true` | Automatically use the Island-native overwrite/delete probe for LocalStorage candidate sets of at least 8 files and 128 KiB of projected key/version/row-ID chunks; set `false` to disable local auto-selection |
