...
def attach_security_routes(
    ...
    *,
    ...
    # --- additional deps for CRUD (previously in rbac.py) ---
    redis_client: Any = None,
    settings: Any = None,
    get_session: Callable = None,
    admin_guard_api: Callable = None,
) -> None:
    """Attach the security page, RBAC redirect, and all role/user CRUD."""
    ...
    # ── Guard: skip CRUD registration if redis_client not provided ────────
    if redis_client is None or admin_guard_api is None:
        return
    ...
    def odata_endpoint_create(
        ...
        # If role_id provided, look up the role to get role_name
        if role_id and not role_name:
            role_doc = _get_role(redis_client, org, sup, role_id)
            if not role_doc:
                raise HTTPException(status_code=404, detail="role not found")
            role_name = role_doc.get("role_name", "")

        doc, token = _create_endpoint(
            redis_client, org, sup,
            label=label, role_id=role_id, role_name=role_name,
        )
        # Include the plaintext token in the response — shown ONCE
