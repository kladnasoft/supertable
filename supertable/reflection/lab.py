...
def attach_lab_routes(
    ...
    *,
    ...
    """Register Lab UI + API routes onto an existing router."""

    @router.on_event("startup")
    async def _lab_startup() -> None:  # pragma: no cover
        await _ensure_worker_started(router)

    @router.get("/reflection/lab", response_class=HTMLResponse)
    def lab_page(
        request: Request,
        ...
        resp = templates.TemplateResponse("lab.html", ctx)
        no_store(resp)
        return resp
    @router.get("/reflection/notebooks")
    def notebooks_redirect(
        ...
        if sup:
            q.append(f"sup={sup}")
        url = "/reflection/lab" + (("?" + "&".join(q)) if q else "")
...
attach_notebook_routes = attach_lab_routes
