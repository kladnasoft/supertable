...
def _sanitize_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Normalize any incoming item to the recommended schema:
      ...
      id, name, kind, size, max_concurrency, is_default

    Unknown fields (like old 'notes') are discarded.
    """
...
def _load(org: str, sup: str) -> Dict[str, Any]:
    ...
    for it in items:
        if not isinstance(it, dict):
            continue
        try:
            # tolerate legacy items (e.g., missing kind/size/max_concurrency/notes)
            sanitized = {
