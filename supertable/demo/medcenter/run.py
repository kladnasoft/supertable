...
def safe_exception_type(error: BaseException) -> str:
    """Return the exception's type name without inspecting its message.

    Teardown reports why a drop was skipped, but the exception text can carry
    storage paths or credentials, so only the inert class name is printed.
    """
