# route: supertable.quality.history
"""
...
#
# Double-underscore-wrap (``__name__``) is the SDK convention for
# internal tables. ``RK._safe()`` accepts this shape; single-wrap
# ``_name_`` is the sentinel pattern and stays reserved. The OData
# handler's ``_is_system_table`` hides ``__*__`` tables from
# user-facing feeds.
...
def write_history_via_sql(
    ...
    """
    try:
        from supertable.quality.config import _dq_key
