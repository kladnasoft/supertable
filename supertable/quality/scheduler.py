...
Redis keys used:
  ...
  supertable:{org}:lakes:{sup}:quality:pending:{table}   — SET by ingest, TTL 10 min
  supertable:{org}:lakes:{sup}:quality:running:{table}   — SET NX by scheduler, TTL 5 min (safety)
  supertable:{org}:lakes:{sup}:quality:cooldown:{table}  — SET by scheduler after check, TTL configurable
"""
